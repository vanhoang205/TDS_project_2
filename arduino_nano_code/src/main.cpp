...
bool triggerTime = true;      // true : 6h - 20h  ** false : 20h - 6h
...
void setup()
{
    ...
    motor1.setSchedule(4 * 1000, 8 * 1000); // GIA TRI MAC DINH DE BAT TAT MOTOR 15MIN-30MIN
...
void loop()
{
    ...
    if (interruptUpdate)
    {
        ...
        {
            if (dhtTemp > 28)
                motor1.setSchedule(15 * 1000, 30 * 1000);
            else
                motor1.setSchedule(15 * 1000, 45 * 1000);
        }
        else
            motor1.setSchedule(15 * 1000, 2 * 60 * 1000);
        ...
        if ((millis() - cloopTime200) > 200)
        {
            ...
            Serial.print((int)tdsValue, DEC);
            Serial.print(",");
            Serial.print((int)temperature, DEC);
            Serial.print(",");
            Serial.print((int)dhtTemp, DEC);
            Serial.print(","); // note that a comma is sent after the last field
            Serial.print((int)dhtHum, DEC);
            Serial.print(","); // note that a comma is sent after the last field
            Serial.print((int)motor1.getState(), DEC);
            Serial.print(","); // note that a comma is sent after the last field
            Serial.print("E"); // send a cr/lf
        }
